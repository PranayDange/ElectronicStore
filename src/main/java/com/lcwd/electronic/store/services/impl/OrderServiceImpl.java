...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDto createOrder(CreateOrderRequest orderDto) {
        String cartId = orderDto.getUserId();
        String userId = orderDto.getCartId();
        //fetch order
        ...
        if (cartItems.size() <= 0) {
            throw new BadApiRequestException("there are not items in cart!!");
        }
    ...
    public void removeOrder(String userId) {
        Order order = orderRepository.findById(userId).orElseThrow(() -> new ResourceNotFoundException("order is not found"));
